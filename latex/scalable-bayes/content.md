...
* Challenges in scaling Bayesian inference algorithms in terms of both model complexity ($p$), and number of observations ($n$)
    ...
    * In practice, many commonly used, practically useful algorithms can have $\mathcal{O}(n^2)$ CPU time scaling behaviour
...
* Immutable data structures acted on with pure functions via combinators such as *map* and *reduce* allow completely automatic parallelisation and distribution of data processing algorithms
	...
	* **Apache Spark** uses an in-memory model, vastly improving performance of algorithms requiring multiple passes over the data - *Spark* is written in the *Scala* programming language, and uses lazy evaluation of computational pipelines to optimise data locality prior to processing (on a cluster)
...
* Provision of performant, flexible, configurable, elastic (private) cloud computing platforms, at scale and reasonable cost
* Requirement for highly multi-disciplinary teams, due to the range of expertise and skills needed
* Lack of suitable, accessible open-data use-cases
* Lack of appropriate tools and libraries

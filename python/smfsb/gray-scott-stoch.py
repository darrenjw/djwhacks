...
for i in range(ts.shape[3]):
    ...
    plt.imsave(f"gss-U-{i:05d}.png", ts[0,:,:,i])
    plt.imsave(f"gss-V-{i:05d}.png", ts[1,:,:,i])
    u_stack.append(io.imread(f"gss-U-{i:05d}.png"))
    v_stack.append(io.imread(f"gss-V-{i:05d}.png"))
print("Creating animated gifs")
io.mimsave("gss-U.gif", u_stack)
io.mimsave("gss-V.gif", v_stack)

...
\frame{
  ...
  \begin{itemize}
  \item The UK National Institute for Data Science and AI --- \url{www.turing.ac.uk}
  \item A partnership of 13 UK Universities with relevant expertise
  \item HQ in the British Library - significant space in Levels 1, 2 an 4
...
\frame{
  \frametitle{Relevent Turing programmes}
  \begin{itemize}
    ...
    \item Health --- PD: Chris Holmes (Oxford) - joint with HDR UK
      ...
      \item Very large remit --- others taking on specific areas --- eg. ``molecular medicine'' (underpinning mechanisms, DJW)
      \item Forming a partnership with UCLH (Sebastian Vollmer, Warwick), and we would be keen to develop another with The Crick
      \end{itemize}
    \item Data science for science --- PD: Jon Rowe (Birmingham)
      \begin{itemize}
      \item Broad remit, including fundamential bioscience research
      \item Currently planning a scoping workshop for ``biology'', with a focus on the LMB and the Crick
      \end{itemize}
  \end{itemize}
}

...
object pfilter {
  ...
  def pfProp(
    ...
    stepFun: (State, Time, Time, Parameter) => State,
    dataLik: (State, Observation, Parameter) => Double,
    data: ObservationTS): (Parameter => (Double,List[State])) = {
    val (times, obs) = data.unzip
    val deltas = diff(t0 :: times)
    (th: Parameter) => {
      val x0 = simx0(n, t0, th)
      @tailrec def pf(ll: Double, x: Vector[List[State]], t: Time, deltas: List[Time], obs: List[Observation]): (Double,List[State]) =
        obs match {

...
Sixth attempt at a very simple canvas drawing app
...
void mand(image *, double complex, double, int);
int level(double complex, int);

int main(int argc, char *argv[]) {
  image *im;
  im = image_alloc(1000, 800);
  mand(im, -2.5 + 1.5*I, 3.0, 60);
  image_write(im, "test6.ppm");
  free(im);
}


void mand(image * im, double complex tl, double i_range, int max_its) {
  int i, j;
  double complex c;
  double res;
  colour col = black;
  ...
  for (i=0;i<(im->w);i++) {
    for (j=0;j<(im->h);j++) {
      ...
      lev = level(c, max_its);
      if (lev == -1) { // in the set
	col.r=0; col.g=0; col.b=20; // very dark blue
      } else { // not in the set
	col.r=255*lev/max_its; col.g=0; col.b=0; // shade of red
      }
      image_set(im, i, j, col);
    }
  }
}

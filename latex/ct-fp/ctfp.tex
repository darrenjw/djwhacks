...
\begin{frame}[fragile]
  ...
  \item If the collection supports a higher-order function\\
    ...
    \end{itemize}
  \item If the collection additional supports a higher-order function\\
...
\begin{frame}[fragile]
  ...
  \item Can be written as a ``for-comprehension'' (``do'' in Haskell):\\
    |s => for (t<-f(s); u<-g(t); v<-h(u)) yield u|
...
\begin{frame}[fragile]
  ...
  \item For now we will think of a monoid as a \alert{set} of elements with a \alert{binary relation} which is \alert{associative}, and having an \alert{identity} element wrt the binary relation

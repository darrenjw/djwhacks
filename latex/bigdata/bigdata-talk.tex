...
\begin{itemize}
\item We live in ``interesting times" --- a time of unparalleled technological advancement --- reason for all the hype
...
\frame{
\frametitle{Statistical methodology for big data}
\begin{itemize}
\item Maximum likelihood
  \begin{itemize}
  \item parallel stochastic gradient descent (SGD)
  \end{itemize}
\item SMC
  \begin{itemize}
  \item parallel resampling
  \item divide and conquer SMC
  \item $\alpha$-SMC
  \item anytime, asynchronous particle filters
  \item decentralised, adaptive, look-ahead particle filters
  \end{itemize}
\item MCMC
  \begin{itemize}
  \item consensus Monte Carlo
  \item Weierstrass sampler
  \item Bayesian conditional density filtering
  \item generalised elliptical slice sampling
  \item sequentially interacting MCMC
  \end{itemize}
\end{itemize}
}
...
\begin{itemize}
\item We are entering the Age of (Big) Data (irrespective of hype)
\item There are \alert{great opportunities} for Statisticians, so long as we are prepared to \alert{keep pace} with technology and \alert{engage}
...
\frame{ 
\frametitle{What could you do with this?}
\centerline{\includegraphics[height=0.8\textheight]{ccii-016}}
}

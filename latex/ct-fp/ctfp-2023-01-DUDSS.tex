...
\title{Functional languages for data science\\(at scale)}
\author{\textbf{\large Darren Wilkinson} \\
\url{@darrenjw}\hfill \alert{\url{darrenjw.github.io}}\\
\vspace{2ex}%
Durham University, UK}
\date{\textbf{DUDSS meeting}\\19th January 2023\\\mbox{}\hfill\alert{\url{github.com/darrenjw/talks}}
}
...
\frame{
\frametitle{Current data science languages}
\begin{itemize}
\item Currently, the most popular programming languages for data science are R and Python
  \begin{itemize}
  \item R is a language specifically designed for statistical computing, and is great for exploratory data analysis, visualisation and statistical modelling
  \item Python is a general purpose programming language that is very effective for data science when used in conjunction with appropriate libraries, and has good interfaces to a variety of specialised machine learning frameworks
    \end{itemize}
  \item All aspiring data scientists should be very familiar with at least one of R and Python (ideally both), and nothing in this talk changes that!
    \item In this talk we will examine the limitations of commonly used languages, and why modern functional programming languages might make a better future data science platform
\end{itemize}
}
...
\frame{
  \frametitle{Ideal languages for statistical computing and ML}
  \begin{itemize}
...
\begin{frame}
  \frametitle{What does ChatGPT think?}
\centerline{\includegraphics[width=\textwidth]{cgpt2}}
\end{frame}
...
\begin{frame}
  ...
  \item The PyMC4 project intended to use TensorFlow for a back-end, but this project was abandoned, and \alert{PyMC(3)} (\url{docs.pymc.io}) is switching from Theano to Aesara with JAX as a back-end
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item Monads are good for sequencing operations that can be carried out on data points independently
    ...
    \item |coflatMap| will take a function representing a \alert{local computation} producing one value for the new structure, and then extend this to generate all values associated with the comonad
      \item Useful for defining (linear) filters, Gibbs samplers, CNNs, time-stepping agents in an ABM, \emph{etc.}
...
\frame{
  ...
  \item To implement solutions to problems in statistical modelling and computation in a more scalable way, we need programming languages which are:
    ...
    \end{itemize}
  \item \alert{Scala} and \alert{Spark} provide a nice illustration of the power of this approach, but there are other interesting languages, including: Haskell, Idris, OCaml, Dex, \ldots
  \end{itemize}
  }
...
\frame{
  ...
  \vfill
\centerline{\url{https://github.com/darrenjw/talks/}}
  \vfill
}

...
\begin{frame}[fragile]
  \frametitle{Monads}
  \begin{itemize}
  \item A \alert{monad} on a category $\mathcal{C}$ is an endofunctor $T: \mathcal{C}\longrightarrow\mathcal{C}$ together with two natural transformations $\eta: \operatorname{Id}_\mathcal{C} \longrightarrow T$ (\alert{lift}) and $\mu: T^2\longrightarrow T$ (\alert{flatten}) fullfilling the \alert{monad laws}:
    \begin{itemize}
    \item \alert{Associativity}: $\mu \circ T\mu = \mu \circ \mu T$, as transformations $T^3\longrightarrow T$
      \item \alert{Identity}: $\mu \circ T\eta = \mu\circ \eta T = 1_T$, as transformations $T\longrightarrow T$
    \end{itemize}
  ...
  \item The identity law says that the two ways of \alert{lifting} $T(x)$ to $T(T(x))$ and then flattening back to $T(x)$ both get back to the original $T(x)$
    \item In FP, we often use |M| (for monad) rather than $T$ (for triple), and say that there are four monad laws --- the additional two laws correspond to the naturality of $\eta$ (lift) and $\mu$ (flatten)
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item \alert{Apache Spark} is a Scala library for Big Data analyics on (large) clusters of machines (in the cloud)
...
\begin{frame}[fragile]
  \frametitle{Lazyness, composition, laws and optimisations}
  \begin{itemize}
    \item Lazyness allows some optimisations to be performed that would be difficult to automate otherwise
...
\begin{frame}[fragile]
  ...
  \item They are more flexible way to provide polymorphic functionality than traditional inheritance-based object classes in traditional object-oriented programming languages

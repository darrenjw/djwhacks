...
abcSmc <- function(N,rprior,dprior,rdist,rperturb,dperturb,factor=10,steps=15,verb=FALSE) {
    priorLW = log(rep(1/N,N))
    priorSample = mclapply(as.list(priorLW),function(x) {rprior()})
    for (i in steps:1) {
        if (verb) message(paste(i,""),appendLF=FALSE)
        out = abcSmcStep(dprior,priorSample,priorLW,rdist,rperturb,dperturb,factor)
        priorSample = out[[1]]
        priorLW = out[[2]]
    }
    if (verb) message("")
...
abcSmcStep <- function(dprior,priorSample,priorLW,rdist,rperturb,dperturb,factor=10) {
    ...
    lw = mcMap( function(th) {
        terms = priorLW + sapply(priorSample, function(x){dperturb(x,th,log=TRUE)})
        mt = max(terms)
...
ss1d <- function(vec)
{
    acs=as.vector(acf(vec,lag.max=2,plot=FALSE)$acf)[2:4]
...
rprior <- function() { runif(3, -4, 2) }
dprior <- function(x, ...) { sum(dunif(x, -4, 2, ...)) }

...
\author[Darren Wilkinson --- CDT Seminar, 19/3/15]{\textbf{\large Darren Wilkinson} \\
\alert{\url{http://tinyurl.com/darrenjw}}\\
School of Mathematics \& Statistics\\Newcastle University, UK}
\date{CCfBD CDT Seminar\\The Core, Newcastle\\19th March 2015}
...
\item Functional programming
 \begin{itemize}
 \item Immutable data structures and referential transparency
 \item Concurrency and parallelisation
 \item Scala
 \item Spark
 \end{itemize} 
\item Summary
...
\frametitle{Scala ecosystem}
\begin{itemize}
\item \alert{Akka} --- actor-based concurrency framework (inspired by Erlang), including \alert{Akka Streams} (\alert{reactive streams})
\item \alert{Spire}/\alert{Algebird}/\alert{Algebra} --- abstract algebra and numerics
\item \alert{Scalding} --- cascading workflow library (from Twitter)
\item \alert{Storm} --- streaming analytics library (from Twitter)
\item \alert{Shapeless} --- typesafe dynamic type support
\item \alert{Scalaz}/\alert{Cats} --- category theory types (functors, monads, etc.)
...
\item \alert{Spark} DataFrames --- new in version 1.3...
\end{itemize}
%Data frames will also be included in the next release of \alert{Spark}...
}
...
\item First import some objects and classes into the namespace:
\end{itemize}
{\scriptsize
\begin{lstlisting}[language=java]
import scala.math._
...
\item Books:
  ...
  \item Learning Spark (Karau et al)
  \item Scala for machine learning (Nicolas) [meh...]
%  \item Several books on Spark just published or currently in press
  \end{itemize}
\item Coursera courses on \alert{FP in Scala} and \alert{Principles of Reactive Programming}
\end{itemize}

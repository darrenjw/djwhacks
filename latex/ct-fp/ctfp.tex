...
\title[CT and FP for scalable statistical computing]{Category theory and functional programming for scalable statistical modelling and computational inference}
...
\begin{itemize}
\item Simulate a collection of parameters, |pars|, from a prior and store in a collection belonging to the generic collection typeclass
...
def pFilter[S: State, O: Observation,
  ...
  x0: C[S], data: D[O], dataLik: (S, O) => LogLik,
    ...
    val updater = update[S, O, C](dataLik,
      stepFun, resample) _
...
\frame{
  ...
  \item To implement solutions to problems in statistical modelling and computation in a more scalable way, we need programming languages which are:
    ...
    \end{itemize}
  \item \alert{Scala} and \alert{Spark} provide a nice illustration of the power of this approach, but there are other interesting languages, including: Haskell, Idris, OCaml, Frege, Eta, ...
  \end{itemize}
  }

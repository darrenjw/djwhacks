...
\end{multline*}
We can re-write this using backshift notation as:
\begin{multline*}
...
\begin{itemize}
\item Returning to discrete stochastic reaction-diffusion, we can have diffusion for each species and a set of reactions for each subvolume
\item For our Lotka Volterra example, we will have 2 species and $3+2\times 4=11$ reactions per subvolume
...
SAME AS PREVIOUS, BUT USING SPATIAL CLE
}

\frame{
\frametitle{SDE approximaton on a fine grid}
SAME AS PREVIOUS, BUT USING A 100X100 GRID
}
...
\item The RDME does not have a continuum limit in general (the length scale of the subvolumes must remain much larger than the reaction distance of bimolecular reactions)

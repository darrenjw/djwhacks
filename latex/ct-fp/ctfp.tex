...
\title{Functional and compositional approaches to scalable statistical computing}
\author{\textbf{\large Darren Wilkinson} \\
Durham University, UK\hfill\alert{\url{darrenjw.github.io}}\\
}
\date{\textbf{Compositional Foundations of Statistics \& Machine learning}\\Warwick University, UK, July 2026}
...
%\hypertarget{compositionality-and-functional-programming}{%
%\section{Compositionality and functional
%programming}\label{compositionality-and-functional-programming}}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item Using |map| to apply a \alert{pure} function to all of the elements in a functorial collection can clearly be done in parallel
...
\begin{frame}
  ...
  \item Python PPLs such as \alert{PyMC} and \alert{NumPyro} (can) use JAX on the backend
  \item Neural network libraries such as \alert{Keras} and \alert{Flax} (can) use JAX as the backend
    \item Since JAX is generally better suited to scientific computing than the other popular ML frameworks, there is a growing scientific computing ecosystem based on JAX
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Running on a single core on my laptop, a JAX version of an MCMC scheme runs over 100 times faster than the same algorithm in regular Python with numpy
    \item \emph{tl;dr} if you have a relatively simple small-to-medium sized problem, and the only thing you care about is speed, somehow express your log-posterior in JAX and then sample it with BlackJAX
  \end{itemize}
\end{frame}
...
\begin{itemize}
\item Simulate a collection of parameters, |pars: C[Param]|, from a prior and store in a generic (functor) collection
...
\begin{frame}[fragile]
  ...
  \item Rather than using a probability monad to represent samples or methods for sampling, one can instead use them to represent the (joint, log) density of the variables
    ...
    \item Can use the joint posterior for simple MH algorithms (and Gibbs, if dependencies are tracked), but for Langevin and HMC algorithms, also need to keep track of gradients, using automatic differentiation (AD)
      \item OK, because (reverse-mode) AD on a compute graph is also computationally tractable using categorical constructs
...
\begin{frame}[fragile]
  ...
  \item Building a probability monad on top of the \alert{free monad} allows this --- implementation of |pure| and |flatMap| is ``suspended'' in a way that allows subsequent interpretation with concrete implementations later
    \item This allows \alert{layering} of multiple inference algorithms, and different interpretation of different parts of the model, enabling various optimisations and sophisticated \alert{composition} of different (hybrid) inference algorithms
    \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Even ``global'' algorithms, such as Langevin and HMC samplers, rely on gradient computations that often have a compositional structure
...
\section{Comonads, recursion schemes and optics}
...
\begin{frame}[fragile]
  ...
  \item Monads are good for sequencing operations that can be carried out on data points independently
    ...
    \item |coflatMap| will take a function representing a \alert{local computation} producing one value for the new structure, and then extend this to generate all values associated with the comonad
      \item Useful for defining linear filters (including discrete Laplacians), Gibbs samplers, convolutional neural networks, etc.
...
\begin{frame}[fragile]
  \frametitle{Lenses (and other optics)}
  \begin{itemize}
  \item An instance of a simple \alert{lens} of type |Lens[S, A]| is just a pair of functions: |get: S => A| and |set: (S, A) => S|
  \item In the context of functional programming, |S| is typically a product type, assumed to contain values of type |A|
  \item Lenses and other related optics are typically used to query and update immutable algebraic data types (ADTs)
  \item Crucially, lenses \alert{compose} (associatively): a |Lens[S, A]| and a |Lens[A, B]| can be composed to get a |Lens[S, B]|
    \item Given |l1: Lens[S, A]| and |l2: Lens[A, B]| we can construct |l3: Lens[S, B]| with |l3.get(s) = l2.get(l1.get(s))| and |l3.set(s, a) = l1.set(s, l2.set(l1.get(s), b))|
    \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Reverse derivative categories}
  \begin{itemize}
    \item The potential applications of optics in scientific and statistical computing extend far beyond simple ADT manipulation
  \item eg. lenses can be used to implement \alert{reverse-mode automatic differentiation}
  \item For a differentiable function $f: S\rightarrow A$, create a |Lens[S, A]| with |get|$(\mathbf{s})=f(\mathbf{s})$ and |set|$(\mathbf{s}',\mathbf{a})=\left(\left.\frac{\partial f}{\partial \mathbf{s}}\right\vert_{\mathbf{s}=\mathbf{s}'}\right)^\top \mathbf{a}$
    \item Then, for a composition of differentiable functions, lens composition implements the back-propagation chain rule
    \end{itemize}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Forwards-backwards algorithms}
  \begin{itemize}
  \item In the context of state space models, various kinds of forwards-backwards algorithms naturally arise
  \item For HMMs and DLMs, Bayesian inference for the hidden state can be computed with a forward pass for filtering, followed by a backward pass for smoothing (using the results of the forward pass)
  \item |get| can be used to compute a forward step, and |set| to implement a backward step
  \item A full algorithm can be implemented by composing the lenses corresponding to each step
    \item The associativity of lens composition can sometimes lead to algorithms that temporally parallelise
    \end{itemize}
\end{frame}
...
\item
  ...
  \item
    ...
    \alert{monads}, \alert{comonads}, and \alert{lenses} provide a useful framework for
...
\bibitem{} Schauer, M., van der Meulen, F.,  Wang, A. Q. (2026) \alert{Compositionality in algorithms for smoothing}, \emph{Compositionality}, \url{https://doi.org/10.46298/compositionality-8-3}.
...
\bibitem{} Shiebler, D., Gavranovic, B., Wilson, P. (2021) \alert{Category theory in machine learning}, \emph{arXiv}, 2106.07032.
...
\bibitem[]{} Milewski, B. (2026) \alert{The dao of functional programming}, \url{https://github.com/BartoszMilewski/DaoFP/}.
%
...
%Further information: \alert{\url{github.com/darrenjw/talks}}
 ...
 % Rainier:
 % \alert{\url{rainier.fit}} \hfill My blog:  \alert{\url{darrenjw.wordpress.com}}

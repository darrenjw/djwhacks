...
object MinPpl {
  ...
  trait Prob[T] {
    ...
    def resample(implicit N: Int): Prob[T] = {
      ...
      Empirical(newParticles.toArray map (pi => Particle(pi.v, law)))
    }
    def cond(ll: T => Double): Prob[T] =
      Empirical(particles map (p => Particle(p.v, p.lw + ll(p.v))))
    def empirical: Array[T] = resample.particles.map(_.v)
  }

  case class Empirical[T](particles: Array[Particle[T]]) extends Prob[T]

  def unweighted[T](ts: Array[T]): Prob[T] = Empirical(ts map (Particle(_, 0.0)))
  ...
  trait Dist[T] extends Prob[T] {
    ...
    def fit(obs: Seq[T]): Prob[T] = Empirical(Array(Particle(obs.head, ll(obs))))
    def fit(obs: T): Prob[T] = fit(List(obs))
    //def fit(obs: T): Prob[T] = Empirical(Array(Particle(obs, ll(obs))))
  }

  case class Normal(mu: Double, v: Double)(implicit N: Int) extends Dist[Double] {
    lazy val particles = unweighted(bdist.Gaussian(mu, math.sqrt(v)).sample(N).toArray).particles
  ...
  case class Gamma(a: Double, b: Double)(implicit N: Int) extends Prob[Double] {
    lazy val particles = unweighted(bdist.Gamma(a, 1.0/b).sample(N).toArray).particles
    def ll(obs: Double) = bdist.Gamma(a, 1.0/b).logPdf(obs)
  }

  case class Poisson(mu: Double)(implicit N: Int) extends Prob[Int] {
    lazy val particles = unweighted(bdist.Poisson(mu).sample(N).toArray).particles

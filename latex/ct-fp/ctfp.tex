...
\frame{
  ...
  \begin{itemize}
  \item A collection of type \texttt{M[T]} can contain (multiple) values of type \texttt{T}
  \item If the collection supports a higher-order function\\ \texttt{map(f: T => S): M[S]} then we call the collection a \alert{Functor}
    \begin{itemize}
    \item eg. \texttt{List(1,3,5,7) map (x => x*2) = List(2,6,10,14)}
    \end{itemize}
  \item If the collection additional supports a higher-order function\\ \texttt{flatMap(f: T => M[S]): M[S]} then we call the collection a \alert{Monad}
    \begin{itemize}
    \item eg. \texttt{List(1,3,5,7) flatMap (x => List(x,x+1)) = \\\hspace{6ex}List(1, 2, 3, 4, 5, 6, 7, 8)}
    \end{itemize}
  \end{itemize}
  }

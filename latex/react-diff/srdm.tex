...
PICTURE HERE (3 adjecent boxes with arrows)
\begin{itemize}
...
\item If $W_{i,t}$ is the Brownian motion governing the flow of mass between cells $i-1$ and $i$, then we actually have
\end{itemize}
\begin{multline*}
dX_{i,t} = D\Delta X_{i,t}\,dt + \sqrt{D (X_{i-1,t}+X_{i,t})}\,dW_{i,t}\\ - \sqrt{D (X_{i,t}+X_{i+1,t})}\,dW_{i+1,t}
\end{multline*}
}
...
\begin{itemize}
\item The noise becomes negligable in high concentration scenarios
...
\frame{
\frametitle{Multivariate SDE}
Also define $\Lambda(X_t) = $
{\scriptsize
\[\begin{pmatrix}\sqrt{X_{N,t}+X_{1,t}}&-\sqrt{X_{1,t}+X_{2,t}}&0&\cdots&0\\0&\sqrt{X_{1,t}+X_{2,t}}&-\sqrt{X_{2,t}+X_{3,t}}&\ddots&0\\\vdots&\ddots&\ddots&\ddots&\vdots\\-\sqrt{X_{1,t}+X_{N,t}}&0&0&\cdots&\sqrt{X_{N-1,t}+X_{N,t}}\end{pmatrix}\]}
Then we can describe diffusion of the lattice with the multivariate SDE
\[
dX_t = D\Delta X_t\,dt +  \sqrt{D}\Lambda(X_t)\,dW_t
\]
}
